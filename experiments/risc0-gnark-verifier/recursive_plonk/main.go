...
// OuterCircuit wraps a BN254 Groth16 inner proof inside a BLS12-381 PLONK outer proof.
...
func main() {
	...
	fmt.Print("Compiling outer circuit (BN254-in-BLS12-381 PLONK)... ")
	t := time.Now()
	...
	fmt.Print("Generating KZG SRS (unsafe, for testing)... ")
	t = time.Now()
	...
	fmt.Print("Setup... ")
	t = time.Now()
	pk, outerVK, err := plonk.Setup(ccs, srs, srsLagrange)
	die("setup", err)
	fmt.Printf("done (%s)\n", time.Since(t))
	...
	fmt.Print("Proving... ")
	t = time.Now()
	outerProof, err := plonk.Prove(ccs, pk, outerWitness)
	die("prove", err)
	fmt.Printf("done (%s)\n", time.Since(t))

	outerPubWitness, err := outerWitness.Public()
	die("public witness", err)
	if err := plonk.Verify(outerProof, outerVK, outerPubWitness); err != nil {
