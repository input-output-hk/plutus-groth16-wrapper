...
| `canonical-inner/sp1-hello-world/` | The canonical SP1 v6 inner-proof bundle (`vk.bin`, `proof.bin`, `public_inputs.bin`, `meta.json`; `n_real = 5`), plus `exit_code.bin`/`vk_root.bin` (the baked version constants). | `vk.bin` from the baked `zkwrap-sp1` VK (regenerated by `cargo run -p zkwrap-sp1 --bin gen-canonical-vk --features gen-vk`, which decompresses SP1's fixed v6.1.0 VK in Rust); the rest derived from the raw `sp1-hello-world/` artifacts. Reproduced byte-for-byte by `zkwrap-sp1::canonicalize`. |
...
SP1 v6.1.0). The canonical bundle is regenerated separately (no `gen-testdata`
...
decompresses SP1's fixed v6.1.0 VK in pure Rust), and
`proof.bin` (= `proof_bytes.bin[100..356]`) / `public_inputs.bin` (the 5 inputs)
/ `meta.json` are derived from the raw `fixtures/sp1-hello-world/` artifacts. See
