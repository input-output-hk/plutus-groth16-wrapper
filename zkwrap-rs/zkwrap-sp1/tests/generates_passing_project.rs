...
//! committed `outer_vk.json` + outer proof; the inner (SP1) layer and the raw
//! SP1 artifacts are shared. The project is written under the workspace
//! `target/` (gitignored) so it can be inspected after the run. If `aiken` is
//! not on `PATH` the test still validates generation and the `aiken check` step
...
fn run_case<P: OuterProof>(outer_mod: &str, vk_rel: &str, proof_rel: &str, out_subdir: &str) {
    ...
    let project = build_validator(&Sp1ValidatorRequest {
        ...
        project_name: "zkwrap/sp1_verifier",
    })
    .unwrap();
    ...
    for (name, src) in [(outer_mod, layer1), ("sp1", layer2), ("verify", validator)] {
        assert!(
