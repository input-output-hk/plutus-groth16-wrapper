//! End-to-end SP1 → Cardano demo, **PLONK outer backend**.
//!
//! Identical to `../sp1-aiken-groth16` except the wrap step proves a PLONK outer
//! proof instead of Groth16 (the inner SP1 proof is the same). Runs the full
//! live pipeline for the `multiply(17, 23)` guest:
//!
//! ```text
...
fn run() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    println!(
        "      ✔ SP1 proof generated; public values = {}",
    ...
    println!(
        ...
        canonical.codegen["sp1_program_vkey_hash"]
            .as_str()
            .unwrap_or("?")
    );
    ...
    println!(
        "\n[3/4] wrap: GnarkCliProver::prove → BLS12-381 PLONK outer proof (loads the proving key, ~30s) …"
    );
    let outer =
