...
> **Current phase:** Phase 5 SP1 plugin landed off-chain: `zkwrap-sp1` mirrors the RISC Zero plugin (`canonicalize` + `Sp1Codegen`/`sp1.ak` + `build_validator`), and the acceptance test generates an Aiken project that `aiken check`s green against committed SP1 fixtures (`fixtures/sp1-hello-world`, `fixtures/canonical-inner/sp1-hello-world`, `fixtures/sp1-outer-proof.json`). The SP1 inner axis: `n_real=2`, `inputs[0]=vkey_hash` (baked), `inputs[1]=committed_values_digest = SHA256(public_values) mod 2^253` (derived on-chain); the fixed v3.0.0 inner VK is baked into the crate; binding via an `ark-groth16` inner verify in `canonicalize`. The core takes raw artifact bytes (no `sp1-sdk` in the default build); a `sp1-sdk`-feature `canonicalize_proof` gives risc0-style native-type ergonomics. Still pending: the live `examples/sp1-aiken-groth16` demo (needs the SP1 prover env), the deferred extraction of the shared outer-test generator into `zkwrap-core`, and Phase 4 step 4 (preview-testnet submission). See [journal.md](journal.md) for latest status.
